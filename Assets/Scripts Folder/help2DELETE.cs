...
public class help2DELETE : MonoBehaviour
{
        ...
        try
        {
            ...
            foreach (Gate_Fence_Database product in productsList)
            {
                if (product.category.Contains("Gate"))
                {
                    ...
                    Button imageButton = productImage.GetComponent<Button>();
                    if (imageButton != null)
                    {
                        // Attach the OnImageClick method to the button's click event
                        imageButton.onClick.AddListener(productInstance.GetComponent<TestingPressDatabase>().OnImageClick);
                    }
                }
            }
        }
